...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    rnn_outputs, rnn_final_state=tf.nn.dynamic_rnn(cell,embed,dtype=tf.float32)
...
def my_optimizer(loss,grad_clip, learning_rate):
    ...
    # use clipping gradients in the range (-5,5)
    clip_gradient=[(tf.clip_by_value(i,-grad_clip,grad_clip),j) for i,j in gradients if i is not None]
    # apply change to the optimizer
    af_optimizer=optimizer.apply_gradients(clip_gradient)
    return af_optimizer
