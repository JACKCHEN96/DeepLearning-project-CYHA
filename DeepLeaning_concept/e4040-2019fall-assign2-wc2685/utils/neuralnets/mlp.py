...
class MLP(object):
    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            #
            if use_bn:
                # we pass in teh parameter we initialized before.
                cache_name="bn_{}".format(i)
                bgamma=params["bn_gamma_{}".format(i)]
                bbeta=params["bn_beta_{}".format(i)]

                # Then we do bn_fw, and pass the parameters with index "i" into it

                x,cache[cache_name]=bn_forward(x,bgamma,bbeta,bn_params[i],"train")
        ...
        for i in range(num_layers - 1):
            ...
            #
            if use_bn:
                # we need to get dx, dgamma, dbeta from backward function.
    ...
    def predict(self, X):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                cache_name="bn_{}".format(i)
                pbeta=params["bn_beta_{}".format(i)]
                pgamma=params["bn_gamma_{}".format(i)]
                x,_=bn_forward(x,pgamma,pbeta,bn_params[i],"test")
