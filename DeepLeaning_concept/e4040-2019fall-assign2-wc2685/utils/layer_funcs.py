...
def conv2d_forward(x, w, b, pad, stride):
    ...
    # We calculte the ouput shape, as mentioned above:
    new_height=((height-filter_height+2*pad)//stride)+1
    new_width=((width-filter_width+2*pad)//stride)+1
    # Init
    out=np.zeros([batch,new_height,new_width,num_of_filters])

    pad_x = np.zeros([batch, height + pad * 2, width + pad * 2, channels])
    pad_x[:, pad:pad + height, pad:pad + width, :] = x
        ...
        for i in range(new_height):
            for j in range(new_width):
                ...
                for idx_channel in range(num_of_filters):
                    # convolution
                    out[idx_batch,i,j,idx_channel]=np.sum(pad_x[idx_batch,num_i:num_i+filter_height, num_j:num_j+filter_width, :]
                                                          * w[:,:,:,idx_channel])+b[idx_channel,]
    return out
    ...
    for idx_batch in range(batch):
        for i in range(new_height):
            for j in range(new_width):
                ...
                for idx_channel in range(num_of_filters):
                    # convolution
                    dw[:,:,:,idx_channel] +=pad_x[idx_batch,num_i:num_i+filter_height, num_j:num_j+filter_width, :]*d_top[idx_batch,i,j,idx_channel]
                    db[idx_channel]=d_top[idx_batch,i,j,idx_channel]
        ...
        for idx_channel in range(channels):
            for i in range(new_height):
                for j in range(new_width):
                    ...
                    num_j=int(j*stride)
                    # convolution
                    out[idx_batch,i,j,idx_channel]=np.max(x[idx_batch,num_i:num_i+pool_size, num_j:num_j+pool_size, idx_channel])
    return out
...
def max_pool_backward(dout, x, pool_size, stride):
    ...
    heightOfpool = pool_size
    widthOfpool = pool_size
    new_height = 1 + (height - heightOfpool) // stride
    new_width = 1 + (width - widthOfpool) // stride
    dx = np.zeros((N_batch, height, width, channels))
        ...
        for c in range(channels):
            for h in range(new_height):
                for w in range(new_width):
                    wd = x[n, h * stride:h * stride + heightOfpool, w * stride:w * stride + widthOfpool, c]
                    zeros = (wd == np.max(wd))
                    dx[n, h * stride:h * stride + heightOfpool, w * stride:w * stride + widthOfpool, c] = dout[n, h, w, c] * zeros
    return dx