...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            momentum_tem=momentums[k]/(1-np.power(beta1,t))
            velocity_tem=velocitys[k]/(1-np.power(beta2,t))
            params[k] -= learning_rate /(np.power(velocity_tem,0.5)+eps)*momentum_tem
