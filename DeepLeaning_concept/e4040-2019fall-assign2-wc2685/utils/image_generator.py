...
class ImageGenerator(object):
    def __init__(self, x, y):
        ...
        # we copy all the parameters needed from above, and set them to 0 or false.
        self.x=x.copy()
        self.y=y.copy()
        ...
        self.trans_height,self.trans_width=0,0
    ...
    def next_batch_gen(self, batch_size, shuffle=True):
        ...
        #######################################################################
        #
        sum_batch=self.N//batch_size
        batch_count=0
        # pass x,y out for safety
        x=self.x
        y=self.y
        while True:
            if(batch_count<sum_batch):
                batch_count+=1
    ...
    def add_noise(self, portion, amplitude):
        ...
        #######################################################################
        #
        num_noise=int(self.N*portion)
        index=np.random.choice(self.N,size=num_noise,replace=False)
        addn=self.x.copy()
        for m in index:
            addn[m,:,:,:]+=int(amplitude*float(np.random.rand(1)))
